...
const ASSETS=[APP_ROOT.toString(),OFFLINE_URL,new URL('styles.css',APP_ROOT).toString(),new URL('app.js',APP_ROOT).toString(),new URL('manifest.json',APP_ROOT).toString(),new URL('icon.svg',APP_ROOT).toString()];
...
function isAppRequest(request){
  ...
  return [APP_ROOT.pathname,`${APP_ROOT.pathname}index.html`,`${APP_ROOT.pathname}app.js`,`${APP_ROOT.pathname}styles.css`].includes(url.pathname);
}
