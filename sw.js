...
const ASSETS=[APP_ROOT.toString(),OFFLINE_URL,new URL('styles.css',APP_ROOT).toString(),new URL('i18n.js',APP_ROOT).toString(),new URL('backup-utils.js',APP_ROOT).toString(),new URL('storage-utils.js',APP_ROOT).toString(),new URL('app.js',APP_ROOT).toString(),new URL('metronome.js',APP_ROOT).toString(),new URL('manifest.json',APP_ROOT).toString(),new URL('icon.svg',APP_ROOT).toString()];
...
function isAppRequest(request){
  ...
  if(request.mode==='navigate') return true;
    return [APP_ROOT.pathname,`${APP_ROOT.pathname}index.html`,`${APP_ROOT.pathname}app.js`,`${APP_ROOT.pathname}metronome.js`,`${APP_ROOT.pathname}i18n.js`,`${APP_ROOT.pathname}backup-utils.js`,`${APP_ROOT.pathname}storage-utils.js`,`${APP_ROOT.pathname}styles.css`].includes(url.pathname);
}
